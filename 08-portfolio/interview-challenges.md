...
## P. The app is live — what that obligates, and what's still open

**The concept.** Open Eventz is live at openeventz.com and available for real parents to use. That one fact sets the bar for everything else: you are in production the moment a link is shareable, not when the checklist behind it is finished.

**What I observed.** During the timezone incident (#18) the site served event times 5–6 hours early. Nothing in the system caught it — the only reason it surfaced is that I opened a page and looked. For a listings product that is the failure that matters most, because a wrong time gets trusted and acted on: a parent plans a morning around it and arrives to nothing.

**How I remedied it.** I moved correctness in front of the write. Every ingest batch is now screened *before* it reaches the database, and the pipeline will publish nothing rather than publish a wrong time (#21). That was the one obligation I couldn't defer once the site was reachable — so it got built first, ahead of everything below.

**What's still open.** The rest of the production bar is written down and honest: a **user-facing "this is wrong" correction mechanism** (top of the list — during #18 the only detection path was me); rate-limiting and abuse detection on engagement; an account-light path for cross-device sync (scoped by risk — see Concept K); scrape monitoring and alerting; and moving the supervision data from "verified where possible" to "verified before anyone relies on it," because a wrong drop-off answer is a child-safety issue, not a data-quality one.

**Summary.** *"It's live, and real parents can use it. The lesson I took from that is you're in production the moment the link is shareable, not when your checklist is done — so you build the irreversible-harm controls first. During a timezone bug the site served event times five hours early, and nothing caught it except me opening a page. So I moved the check in front of the database write: the ingest now refuses to publish rather than publish a wrong time. What's still open is written down too — a user-facing 'this is wrong' correction path is top of that list, then abuse controls, scrape alerting, and moving the child-supervision data to a 'verified before anyone relies on it' bar."*
